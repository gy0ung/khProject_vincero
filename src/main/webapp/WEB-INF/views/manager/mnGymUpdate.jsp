...
<div
	...
	<form name="frm" method="post"
		action="${pageContext.request.contextPath}/manager/mnGymUpdate.mn">
		...
		<table>
			...
			<tr align="center">
				<td><input type="text"
					style="border: 0 solid black; width: 70px;" id="gymNo" name="gymNo"
					value="${gym.gymNo}" readonly></td>
				<td><input type="date" id="gymRegdate" name="gymRegdate"
					style="width: 130px;" ${gym.gymStatus eq 2 ? '' : 'disabled'} value="${gym.gymRegdate}"></td>

				<td><input type="text" id="businessNo" name="businessNo"
					style="width: 140px;" value="${gym.businessNo}"></td>
				<td><input type="text" id="gymBoss" name="gymBoss"
					style="width: 90px;" value="${gym.gymBoss}"></td>
				<td><input type="text" id="gymName" name="gymName"
					style="width: 140px;" value="${gym.gymName}"></td>
				<td><select name="gymType" style="width: 90px;">
						...
						<option value="striker"
							${gym.gymType eq 'striker' ? 'selected' : '' }>킥복싱류</option>
						<option value="box" ${gym.gymType eq 'box' ? 'selected' : '' }>복싱</option>
				</select></td>
				<td><input type="text" id="matchCount" name="matchCount"
					style="width: 60px;" value="${gym.matchCount}"></td>
				<td><select name="gymStatus"
					onchange="handleGymStatusChange(this)">
				...
				<td>
					...
					<input type="${gym.gymStatus eq 4 ? 'date' : 'text'}" id="reason"
					name="reason" value="${gym.reason}"
					${gym.gymStatus eq 3 || gym.gymStatus eq 4 ? '' : 'disabled'}>
				</td>
				<td><c:if test="${gym.gymStatus ne 2}">
						일반
					</c:if> <c:if test="${gym.gymStatus eq 2}">
						관장님
					</c:if></td>
				<td><c:if test="${gym.userId == null}">
					</c:if> <c:if test="${gym.userId != null}">
						${gym.userId}
		...
		<div id="btnmem">
			<button type="button" style="width: 80px"
				onclick="location.href='${pageContext.request.contextPath}/manager/mnGymList.mn'">목록으로</button>
			<button type="button" style="width: 110px" onclick="gymToggle()">상세정보
				보기</button>
		...
		<!--  체육관 상세보기 -->
		<h1 id="gymH1" style="display: none;">no.${gym.gymNo}&ensp;체육관
			상세정보 수정</h1>
		<table id="gymTable" style="display: none;">
			...
			<tr>
				<td>${gym.gymCoach}
				<input type="hidden" id="gymCoach" name="gymCoach"
					style="width: 90px;" value="${gym.gymCoach}"></td>
				<td>
				${gym.coachPhone}
				<input type="hidden" id="coachPhone" name="coachPhone"
					style="width: 140px;" value="${gym.coachPhone}"></td>
				<td><input type="text" id="gymZipcode" name="gymZipcode"
					style="width: 80px;" value="${gym.gymZipcode}">
					<button onclick="findAddr();">찾기</button></td>
				<td><input type="text" id="gymAddress" name="gymAddress"
					style="width: 240px;" value="${gym.gymAddress}" readonly></td>
				<td><input type="text" id="gymDetailaddress"
					name="gymDetailaddress" style="width: 210px;"
					value="${gym.gymDetailaddress}"></td>
				<td><input type="text" id="gymPhone" name="gymPhone"
					style="width: 140px;" value="${gym.gymPhone}"></td>
				<td><input type="text" id="gymBank" name="gymBank"
					style="width: 100px;" value="${gym.gymBank}"></td>
				<td><input type="text" id="gymAccount" name="gymAccount"
					style="width: 165px;" value="${gym.gymAccount}"></td>
			</tr>
		</table>
	</form>
</div>
...
<!-- 체육관 상태에 대한 스크립트 -->
	function handleGymStatusChange(selectElement) {
		...
		var dateInput = document.getElementById('gymRegdate');
		var selectedValue = selectElement.value;

		// 계약체결(2)을 선택하였을때 <input>태그 타입이 날짜로 변경되게 하는 스크립트
		if (selectedValue === '2') {
			dateInput.disabled = false; // 비활성화를 false함
			// date형식으로 날짜를 입력받지만 DB에 reason은 varchar2이므로 String으로 저장됨
		} else {
			dateInput.disabled = true; // 비활성화를 true함
		}		
