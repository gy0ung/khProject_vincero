...
<jsp:include page="/WEB-INF/views/common/header.jsp">
	<jsp:param value="빈체로 - 매치등록" name="title" />
</jsp:include>

<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=b1a1baddfc194b964c714fcbe3f6d1aa&libraries=services"></script>
<script src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/match/matchReg.css?v=<%=System.currentTimeMillis()%>">
...
<script>
	...
	function matchList() {
		const url = "${pageContext.request.contextPath}/match/matchList.ma";
        location.href = url;
	}
	</script>
			...
			<tr>
				<c:forEach items="${dateFilter}" var="item" varStatus="d">
					<c:if test="${item.day lt item.today}">
						<td class="pre"><span>${item.yoil}</span>
							<p>${item.date}</p></td>
					</c:if>

					<c:if test="${item.day ge item.today}">
						<td class="dateform" id="dateFilter"
							onclick="dateFilter('${item.day}');"><a href="#map"></a> <span>${item.yoil}</span>
							<p>${item.date}</p></td>
					</c:if>
	...
	<div id="schedule">
		<form>
			<table id="scheduleT">
				<h2>선택한 일정 리스트</h2>
				<tr>
					<th colspan="3">삭제 후 등록버튼을 눌러주세요!</th>
					<th><button class="scheduleButton" type="button"
							onclick="registerMatch();">등록하기</button></th>
				</tr>
				<tr></tr>
				<tr>
					<th>체육관 이름</th>
				...
				<tbody id="register">
					<c:forEach items="${inSchedList}" var="inSchedule">
						<tr id='${inSchedule.code}'>
							...
							<td><button type="button" id="delete" style="align: right;"
									onclick='deleteTime(`${inSchedule.code}`);'>삭제</button></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
